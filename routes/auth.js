...
router.post("/register", async (req, res) => {
  try {
    ...
    // Clones the data in Saveduser to a document so we can delete the password
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ error: "There was a problem with the login" });
    }
...
// @desc    Logout user and clear cookkkie
// @access  Private
